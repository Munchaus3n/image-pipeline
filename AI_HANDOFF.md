...
- Editor visual redo is implemented on the clean task branch: compact top command bar, display-only zoom multiplier `0.74`, centered sharp-corner canvas viewport capped to fit at 100% without vertical canvas scrolling, flat 300px accordion right sidebar with compact controls and no colored section indicators, separate Scale section, compact Items/Output panels, no footer stage chips, filename in the footer, queue chip beside Skip, and no System/GPU card.
...
- Do not start Templates or Settings visual redesign unless explicitly scoped.
