...
// zoom: CSS scale applied to the canvas element for Phase 4 view zoom
...
export default function App({ onGoPipeline, outputDir = "", canvasSize: canvasSizeProp = null, thumbnail: thumbnailProp = true }) {
  ...
  useEffect(() => {
    ...
    // Active template zone: solid border + subtle fill, drawn on top of everything
  ...
  return (
    ...
    >
      ...
      <div style={{ padding:"12px 8px 12px 12px", display:"flex", flexDirection:"column", gap:8 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", height:22 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <span style={{ fontSize:9, color:C.dim2, fontFamily:"JetBrains Mono" }}>{CANVAS_SIZE} × {CANVAS_SIZE}</span>
            <button onClick={onGoPipeline} style={{
              background:"transparent", color:C.dim, border:`1px solid ${C.border}`,
              borderRadius:4, padding:"3px 10px", fontSize:10, cursor:"pointer", fontFamily:"inherit",
            }}>← Pipeline</button>
          </div>
        </div>

        {/* Canvas wrapper — overflow hidden so zoomed canvas clips cleanly */}
        <div style={{
          width: DS, height: DS,
          overflow: "hidden",
          flexShrink: 0,
          ...
          <canvas
            ...
            style={{
              ...
              // Phase 4: CSS scale — keeps pixel buffer at DS×DS, visually zooms
              width:  DS * zoom,
              height: DS * zoom,
      ...
      <div style={{
        ...
        <button onClick={snapToFull} style={{
          width:"100%", background:C.panel2, color:C.text,
          border:`1px solid ${C.border}`, borderRadius:4,
          padding:"5px 8px", fontSize:11, cursor:"pointer",
          fontFamily:"inherit", marginBottom:5, textAlign:"center",
        }}>⬛ Fit Full Canvas</button>
        ...
        {/* Phase 4: View zoom ─────────────────────────────────────────────── */}
        <Divider label="View Zoom" />
        <div style={{ display:"flex", gap:3, marginBottom:4 }}>
          {zoomPresets.map(({ label, value }) => (
            <button key={label} onClick={() => setZoom(value)} style={{
              flex:1, padding:"4px 0", fontSize:10, cursor:"pointer",
              fontFamily:"inherit", borderRadius:4,
              background: zoom === value ? C.panel2 : "transparent",
              color:       zoom === value ? C.text   : C.dim,
              border:      `1px solid ${zoom === value ? C.border : "transparent"}`,
              fontWeight:  zoom === value ? 600 : 400,
            }}>{label}</button>
          ))}
        </div>
        <div style={{ fontSize:9, color:C.dim, paddingLeft:2 }}>
          Visual only — does not affect export size.
        </div>

        {/* Phase 2: Output folder ─────────────────────────────────────────── */}
        <Divider label="Output" />
        <Btn onClick={() => openFolder()} style={{ color:C.green, borderColor:"#1a3a20" }}>
          📁 Open Output Folder
        </Btn>
